...
class MicrodataTestData:
    ...
    data_temporality_type = "STATUS"  # Use "STATUS", "EVENT" or "ACCUMULATED"
    min_start_date = datetime.date(2000, 1, 1)
    max_stop_date = datetime.date(2020, 12, 31)
    list_of_status_dates = [
    ...
    def generate_test_dataset(self, file_name, value_type="CODE_VALUES"):
        ...
        rows = []
        unit_id = ''.join(random.choice(self.__all_digits) for i in range(14))
        num_of_status_dates = len(self.list_of_status_dates)
        cnt_status = 0
        for i in range(0, self.number_of_rows_in_testfile):
            if self.data_temporality_type in("EVENT", "ACCUMULATED"):
                rows.append(self.generate_data_row(value_type))
            else:
                # data_temporality_type == "STATUS"
                if cnt_status < num_of_status_dates-1:
                    cnt_status += 1
                else:
                    unit_id = ''.join(random.choice(self.__all_digits) for i in range(14))
                    cnt_status = 0
                value = self.legal_code_values[random.randint(0, len(self.legal_code_values)-1)]
                start = self.list_of_status_dates[cnt_status]
                stop = self.list_of_status_dates[cnt_status]
                status_row = \
                    str(unit_id) + self.data_field_separator \
                    + value + self.data_field_separator \
                    + start + self.data_field_separator \
                    + stop
...
# mtd.data_temporality_type = "STATUS"  # Use "STATUS", "EVENT" or "ACCUMULATED"
# mtd.list_of_status_dates = ["2000-12-31"]
...
## Generate dataset with code-list values and temporalityType="EVENT" (event history data with start and stop/end date)
# mtd = MicrodataTestData()
# mtd.file_path = "C:/BNJ/prosjektutvikling/Python/micordata-datastore/temp"
# mtd.number_of_rows_in_testfile = 1000
# mtd.include_header = False
# mtd.data_field_separator = ";"
# mtd.attribute_fields = None
# data_temporality_type = "EVENT"  # Use "STATUS", "EVENT" or "ACCUMULATED"
# mtd.min_start_date = datetime.date(1970, 1, 1)  # Same start and stop for status-datasett
# mtd.max_stop_date = datetime.date(2020, 12, 31)
# mtd.legal_code_values = ["A", "B", "C", "D"]
# mtd.generate_test_dataset(file_name="testdata_1000.txt", value_type="CODE_VALUES")


## Generate dataset with code-list values and temporalityType="STATUS"
mtd = MicrodataTestData()
mtd.file_path = "C:/BNJ/prosjektutvikling/Python/micordata-datastore/temp"
mtd.number_of_rows_in_testfile = 1000000
mtd.include_header = False
mtd.data_field_separator = ";"
mtd.attribute_fields = None
mtd.data_temporality_type = "STATUS"  # Use "STATUS", "EVENT" or "ACCUMULATED"
mtd.list_of_status_dates = [
        "2000-12-31", "2001-12-31", "2002-12-31", "2003-12-31",
        "2004-12-31", "2005-12-31", "2006-12-31", "2007-12-31",
        "2008-12-31", "2009-12-31"]
#mtd.min_start_date = datetime.date(1970, 1, 1)  # Same start and stop for status-datasett
#mtd.max_stop_date = datetime.date(2020, 12, 31)
mtd.legal_code_values = ["A", "B", "C", "D"]
mtd.generate_test_dataset(file_name="testdata_1_million_STATUS.txt", value_type="CODE_VALUES")
